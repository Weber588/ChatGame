...
public class Game {
	...
	private static TriviaQuestion custom = null;
	private static int customPoints = 0;
	...
	public static void start() {
		...
		RandomMinecraft.setUnscramble(unscramble);
		RandomMinecraft.setTrivia(trivia);
		RandomMinecraft.setQuicktype(quicktype);
		RandomMinecraft.setReveal(reveal);
	...
	public static void setCustom(TriviaQuestion newCustom, int points) {
		custom = newCustom;
		customPoints = points;
	}
	...
	private static String getRandomWord() {
		...
		if (genType == null) { // Use provided list
			...
			String word = RandomMinecraft.getTypeWord(currentType);
			return word;
			...
			default: // Use provided word list
				autoGeneratedWord = false;
				return RandomMinecraft.getTypeWord(currentType);
			}
	...
	private static void unscramble() {
		...
		if (challenge != null && doChallenges) { // Use a challenge
	...
	private static void math() {
		...
		if (usePedmas) {
			...
			total = solvePedmas(questionString);
	...
	private static double solveLeftToRight(String equation) {
		String[] components = equation.split(" ");
		double total = Double.parseDouble(components[0]);
		
		for (int i = 0; i < components.length; i++) {
			if (components[i].equals("+")) {
				total += Double.parseDouble(components[i+1]);
	...
	private static double solvePedmas(String equation) {
		int escape = 0;
		while ((equation.contains("*") || equation.contains("/")) && escape < 100) {
			escape++;
			
			String[] components = equation.split(" ");

			int index = 0;
			calculations: for (String comp : components) {
				if (comp.equals("*")) {
					
					double[] prod = new double[2];
					prod[0] = Double.parseDouble(components[index-1]);
					prod[1] = Double.parseDouble(components[index+1]);
					
					if ((prod[0] > 3 && prod[1] > 3) || (prod[0] > 15)) {
						currentPoints += (int) prod[1] / 3;
					}
					
					String productString = prettyDoubleString(prod[0]) + " * " + prettyDoubleString(prod[1]);
					
					double val = prod[0] * prod[1];
					
					equation = equation.replace(productString, prettyDoubleString(val));
					
					break calculations;
				} else if (comp.equals("/")) {
					double[] div = new double[2];
					div[0] = Double.parseDouble(components[index-1]);
					div[1] = Double.parseDouble(components[index+1]);

					if (div[0] % div[1] != 0) {
						currentPoints++;
					}
					
					String divString = prettyDoubleString(div[0]) + " / " + prettyDoubleString(div[1]);

					double val = div[0] / div[1];
					
					equation = equation.replace(divString, prettyDoubleString(val));
		...
		return solveLeftToRight(equation);
		
	}
	
	private static String prettyDoubleString(double val) {
		String pretty = Double.toString(val);
	...
	private static void trivia(int challengeFailCount) {
		...
		if (challenge != null && custom == null && doChallenges) { // Use a challenge
			...
			TriviaQuestion chosenQuestion = RandomMinecraft.randomTrivia();
			if (custom != null) {
				chosenQuestion = custom;
			...
			if (custom == null) {
				currentPoints = trivia.get(chosenQuestion);
			} else {
	...
	private static void quicktype() {
		...
		if (challenge != null && doChallenges) {
			switch (challenge.toUpperCase()) {
			case "BACKWARDS":
